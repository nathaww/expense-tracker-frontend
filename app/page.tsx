...
const FeatureCard = ({ icon, title, description }: { icon: React.ReactNode, title: string, description: string }) => (
  <motion.div
    ...
    className="bg-[var(--bgSecondary)] rounded-[var(--border-radius)] p-6 border border-[var(--border-color)] hover:border-[var(--color-primary)]/20 hover:shadow-lg transition-all group"
  >
...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen relative">
      <div className="h-full w-full flex flex-col justify-center items-center px-4 py-20 sm:py-32 bg-[var(--bg)]">
        ...
        <div className=" z-10 flex flex-col items-center max-w-6xl mx-auto">
          ...
          <motion.span
            ...
            transition={{ duration: 0.5 }}
            className="mb-6 inline-flex items-center gap-1.5 rounded-full bg-[var(--text)]/10 px-4 py-1.5 text-sm border border-[var(--text)]/10 text-[var(--text)]"
          >
      ...
      <div className="fixed inset-0 w-full h-full z-1 opacity-20">
        <AuthPattern />
      </div>
    </div>
  );
