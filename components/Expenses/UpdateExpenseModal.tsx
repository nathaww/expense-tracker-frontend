...
export const UpdateExpenseModal = ({ expense }: UpdateExpenseModalProps) => {
  ...
  const { data: moneySourcesResponse, isLoading: moneySourcesLoading } = useQuery({
    queryKey: ["money-sources-for-expenses"],
    queryFn: expenseRequests.getMoneySources,
  });
  
  const moneySources = moneySourcesResponse?.data || [];
  const isDataLoading = categoriesLoading || moneySourcesLoading;
  ...
  // Ensure we have the correct string values for IDs
  const ensureStringId = (id: string | number | null | undefined): string => {
    if (id === null || id === undefined) return "";
    return String(id);
  };

  // Use useMemo to ensure initial values are only recalculated when expense changes
  const initialValues: PostCategory = useMemo(() => ({
    amount: expense.amount,
    date: formatDateForInput(expense.date),
    notes: expense.notes || "",
    categoryId: ensureStringId(expense.categoryId),
    moneySourceId: ensureStringId(expense.moneySourceId),
  }), [expense]);
    ...
    <Dialog.Root open={isOpen} onOpenChange={setIsOpen}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[95%] sm:w-full max-w-md bg-[var(--bg)] rounded-[var(--border-radius)] shadow-xl p-4 sm:p-6 z-50 max-h-[90vh] overflow-y-auto">
          ...
          {/* Only render Formik when we have all the data */}
          {isDataLoading ? (
            ...
            <Formik
              ...
              enableReinitialize
            >
              {({ errors, touched }: { errors: FormikErrors<PostCategory>; touched: FormikTouched<PostCategory>; values: PostCategory }) => {
                ...
                return (
                  <Form className="space-y-4">        
                    ...
                    {/* Amount */}
                    <div>
                      <label htmlFor="amount" className="block text-[var(--text)] mb-2">
                        Amount *
                      </label>
                      <Field
                        type="number"
